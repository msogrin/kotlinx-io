...
internal fun <T> ioJson.readJson(element: JsonElement, deserializer: DeserializationStrategy<T>): T {
...
private sealed class AbstractJsonTreeInput(override val json: ioJson, open val obj: JsonElement) : NamedValueDecoder(),
    ...
    override fun decodeTaggedChar(tag: String): Char {
        ...
        return if (o.content
                .length == 1
        ) o.content[0] else throw SerializationException("$o can't be represented as Char")
    }
...
private class JsonPrimitiveInput(json: ioJson, override val obj: JsonPrimitive) : AbstractJsonTreeInput(json, obj) {
    ...
    override fun currentElement(tag: String): JsonElement {
        require(
            tag === PRIMITIVE_TAG
        ) { "This input can only handle primitives with '$PRIMITIVE_TAG' tag" }
        return obj
    }
}

private open class JsonTreeInput(json: ioJson, override val obj: JsonObject) : AbstractJsonTreeInput(json, obj) {
    ...
    override fun endStructure(desc: SerialDescriptor) {
        ...
        for (key in obj.keys) {
            if (key !in names) throw ioJsonUnknownKeyException(-1, key)
        }
    }
}

private class JsonTreeMapInput(json: ioJson, override val obj: JsonObject) : JsonTreeInput(json, obj) {
...
private class JsonTreeListInput(json: ioJson, override val obj: JsonArray) : AbstractJsonTreeInput(json, obj) {
